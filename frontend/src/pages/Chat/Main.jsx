...
import { sendMessage, checkConnection, connectSocket, disconnectSocket, joinRoom, getSocket } from '../../lib/socket';
    ...
    return (
        <div className="flex flex-col justify-between h-screen">
            ...
            <div className="h-fit bg-base-100 overflow-y-auto">
                ...
                <div className="flex-1 overflow-y-auto p-4 space-y-2">
                    {messages.map((msg, idx) => (
                        <div
                            key={idx}
                            className={`flex flex-col ${msg.senderId === user ? 'items-end' : 'items-start'}`}
                        >
                            <div
                                className={`px-4 py-2 rounded-lg max-w-xs ${
                                    msg.senderId === user ? 'bg-blue-500 text-white' : 'bg-slate-600/90 text-white'
                                }`}
                            >
                                {msg.content}
                            </div>
            ...
            <div className="p-4 pr-6 flex gap-6">
                <button className="p-3 border-2 border-stone-700/40 rounded-lg cursor-pointer">
                    <Link size={30} />
                </button>
                <input
                    type="text"
                    className="w-full py-3 px-6 text-xl rounded-l-4xl rounded-r-2xl bg-white/20 text-white/90 outline-none"
                ...
                />
                <button className="px-3 border-2 border-slate-800/80 rounded-lg cursor-pointer text-xl font-medium tracking-wider">
                    GIF
                </button>
                <button
                    onClick={() => handleSend()}
                    className="p-3 px-5 border-2 border-slate-800/80 rounded-lg cursor-pointer"
                >
                    <Send size={30} />
                </button>
            </div>
